...
int init_client_threads(pthread_t *tids, client_thread_params_t *thread_params, int client_socket, struct sockaddr_in *server_endpoint, logs_file_t  *client_logs_file, int packets_per_thread,void *(*handle_client) (void *)){
    ...
    for (uint32_t i = 0; i < NUMBER_OF_SENSORS; i++){
        ...
        thread_params[i].client_logs_file = client_logs_file;
